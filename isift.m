...
function [feature, mag]=isift(img)

row=128;
colum=128;
%img=imresize(I,[row,colum]);
...
% hold on
% plot(ry,rx,'r+');
% title('Initial Key Points Localization');
...
% hold on
% plot(ry,rx,'b+');
% title('Accurate Key Points Localization using SIFT');
...
feature=(feature)' ;
mag=zeros(size(feature,1),1);
for oo=1:size(feature,1)
    mag(oo)=norm(feature(oo,:));
end
